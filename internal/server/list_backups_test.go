...
func TestMakeListBackupsHandler(t *testing.T) {
	tests := []struct {
		name       string
		...
		},
		{
			...
			args: listBackupsArgs{Search: "nvd"},
			clientResp: []client.Entry{
				{Name: "vulncheck-nvd2", Description: "NVD CVE data"},
				{Name: "initial-access", Description: "Initial access exploits"},
			},
		...
		},
		{
			...
			args: listBackupsArgs{Search: "INITIAL"},
			clientResp: []client.Entry{
				{Name: "vulncheck-nvd2", Description: "NVD CVE data"},
				{Name: "initial-access", Description: "Initial access exploits"},
			},
		...
		},
		{
			...
			args: listBackupsArgs{Search: "nonexistent"},
			clientResp: []client.Entry{
				{Name: "vulncheck-nvd2", Description: "NVD CVE data"},
			},
			wantTotal: 0,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mock := &mockClient{
				listBackupsFn: func(_ context.Context) ([]client.Entry, error) {
					return tt.clientResp, tt.clientErr
				},
			}

			handler := MakeListBackupsHandler(mock)
